...
WORKFLOW = "p37-fourier-phase-bispectrum-audit.yml"


PRESETS = {
    "remote-smoke": {
        "run_label": "p37-fourier-phase-bispectrum-smoke",
        "sample_sizes": "10",
        "metric_mode": "smoke",
        "max_candidates": "2",
        "max_tasks": "2",
        ...
        "multiplier_minimized": "false",
        "shard_count": "1",
        "max_parallel": "1",
    },
    "p37-fourier-phase-bispectrum-40": {
        "run_label": "p37-fourier-phase-bispectrum-40x",
        "sample_sizes": "100,300,500,1000,full",
        "metric_mode": "full",
        "max_candidates": "0",
        "max_tasks": "0",
...
def validate(values):
    ...
    if values["diagnostic_type"] not in ("sampled", "full"):
        raise ValueError("diagnostic_type must be sampled or full")
    if str(values["multiplier_minimized"]).lower() not in ("true", "false"):
        raise ValueError("multiplier_minimized must be true or false")
    for key in ("max_candidates", "max_tasks", "diagnostic_sample_count"):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build or dispatch p37 Fourier phase bispectrum audit runs.")
