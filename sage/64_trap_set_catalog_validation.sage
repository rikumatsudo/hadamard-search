...
P167_SCORE_SET = set([164, 176, 184, 200, 216, 232])
...
def collect_p167_candidates(max_total):
    roots = [
        "outputs/explorations/20260506_0630_hadamard668_coherent_multiswap_escape",
        "outputs/explorations/20260506_0840_hadamard668_escapability_aware_guided_search",
        "outputs/explorations/20260506_0840_hadamard668_escapability_aware_guided_search_patch_smoke",
        "outputs/explorations/20260506_0430_hadamard668_score_neighborhood_census",
        "outputs/explorations/20260506_0230_hadamard668_moment_compatible_pool_mitm_A_medium",
        "outputs/explorations/20260506_0205_hadamard668_moment_compatible_pool_mitm_smoke",
        "outputs/candidates",
    ]
    files = []
    for root in roots:
    ...
    for path in files:
        for payload in iter_payloads(path):
            ...
            counts = S62["total_diff_counts"](p, blocks)
            score = S62["score_counts"](counts, lam)
            if int(score) not in P167_SCORE_SET and not (150 <= int(score) <= 240):
                continue
            h = S62["canonical_hash"](blocks, ks, p)
            occurrences.setdefault(h, []).append({"source_file": path, "source_mode": payload.get("mode"), "source_label": payload.get("label")})
            if h not in first:
                first[h] = {
                    "p": 167,
                    "ks": [int(k) for k in ks],
    ...
    rows = sorted(first.values(), key=lambda r: (r["score"], r["source_file"], r["canonical_hash"]))
    return rows[: int(max_total)], occurrences
...
def feature_row(candidate, response_by_hash, occurrences, baseline_cache):
    p = int(candidate["p"])
    ...
    counts = S62["total_diff_counts"](p, blocks)
    diag = S62["full_diagnostic"](blocks, counts, lam, p, baseline, powers=POWERS_DEFAULT)
...
def main():
    ...
    parser.add_argument("--max-668-candidates", type=int, default=80)
    ...
    p167_rows, occ_167 = collect_p167_candidates(args.max_668_candidates)
    response_rows = response_join_rows()
    response_by_hash = summarize_response_by_hash(response_rows)
    baseline_cache = {}
    p37_features = [feature_row(row, response_by_hash, occ_p37, baseline_cache) for row in selected_p37]
    nearhit_features = [feature_row(row, response_by_hash, occ_167, baseline_cache) for row in p167_rows]
