...
def init_blocks(tuple_row, seed_family, run_seed, args):
    ...
    for i in range(pool_size):
        ...
        if best is None or key < best[0]:
            best = (key, blocks)
    return best[1], "mixed_diversity_score_support_pool{}".format(pool_size)
...
def task_grid(tuple_rows, seed_families, operators, restarts, seed_base):
    tasks = []
        ...
        for seed_family in seed_families:
            for operator in operators:
                for restart_id in range(int(restarts)):
                    ...
                    task_id = "p167_stage0_" + stable_hash(raw)[:16]
...
def run(args):
    ...
    if args.aggregate_roots:
        ...
        tasks_all = task_grid(tuple_registry, seed_families, operators, int(args.restarts), int(args.seed_base))
    ...
    print("Wrote Stage 0 calibration outputs to {}".format(args.out_dir))
