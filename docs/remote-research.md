...
SageMath実験、またはRust探索エンジン + Sage厳密検証をGitHub Actions上で実行できます。
...
Rust探索エンジンで同じsmokeを走らせる場合は `engine=rust` にします。
Rust側は候補生成とscore計算までを担当し、score0候補だけをSage verifierへ渡します。

```bash
env -u GITHUB_TOKEN gh workflow run research.yml \
  -f run_label=p167-rust-actions-smoke \
  -f engine=rust \
  -f config=configs/experiments/p167_tuple_A_actions_smoke.yaml \
  -f seeds=1 \
  -f steps=1 \
...
本番向けの具体的なdispatch手順は
[docs/production-runs.md](production-runs.md) に固定しています。
...
GitHub Proのstandard runnerでは、現時点では40 concurrent jobsが目安です。
...
`total_seeds` 個のseedを `shard_count` 等分し、各jobに `shard_index` を渡します。
例えば200 seedsを40等分する場合、1 jobあたり5 seedsになります。

40 shardを1つのworkflow runでfan-outする例:

```bash
env -u GITHUB_TOKEN gh workflow run research.yml \
  -f run_label=p167-prod-40x \
  -f engine=rust \
...
20等分にする場合は、`shard_count=20` と `max_parallel=20` に変えます。

1 shardだけ再実行する例:

```bash
env -u GITHUB_TOKEN gh workflow run research.yml \
  -f run_label=p167-prod-40x-rerun-s00 \
  -f engine=rust \
  -f config=configs/experiments/p167_tuple_A_rust_production.yaml \
  -f total_seeds=400 \
  -f shard_count=40 \
...
各shardは個別artifactをuploadし、最後にaggregate jobがbest score、
score0 candidate数、Sage検証済み候補数、成功shard数をまとめます。
Slack通知もaggregate結果として1回だけ届きます。
aggregate artifactには `aggregate_summary.md`, `aggregate_summary.json`,
`aggregate_slack_payload.json` が含まれます。
`snapshot_interval` や `max_repair_candidates` はSage runner向けの入力です。
Rust MVPではfrontier/repairをまだ持たないため、主に `total_seeds`, `shard_count`,
`shard_index`, `steps` で探索量を制御します。
